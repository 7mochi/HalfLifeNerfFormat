...
#define CLS_HEALTHKIT  			"item_healthkit"
...
new Float:g_BatOrigin[MAX_ITEMS][3];
new Float:g_BatAngles[MAX_ITEMS][3];
new g_BatCount;

new Float:g_KitOrigin[MAX_ITEMS][3];
...
public plugin_init()
{
	...
	bind_pcvar_num(create_cvar("amx_nerf_swap_batteries_with_healthkits", SWAP_BATTERIES_WITH_HEALTHKITS_DEFAULT_VALUE, FCVAR_NONE, "Swap Battery Items with Health Kit Items on the Ground.", true, 0.0, true, 1.0), g_bSwapBatteriesWithHealthKits);
	bind_pcvar_num(create_cvar("amx_nerf_swap_healthkits_with_batteries", SWAP_HEALTHKITS_WITH_BATTERIES_DEFAULT_VALUE, FCVAR_NONE, "Swap Health Kit Items with Battery Items on the Ground.", true, 0.0, true, 1.0), g_bSwapHealthKitsWithBatteries);
	bind_pcvar_num(create_cvar("amx_nerf_start_armor", HEV_START_DEFAULT_VALUE, FCVAR_NONE, "Armor players should start with.", true, 0.0, true, 100.0), g_iStartArmor);
...
{
	if(g_bSwapBatteriesWithHealthKits)
		CollectAndRemove(CLS_BATTERY,   g_BatOrigin,  g_BatAngles,  g_BatCount);

	if(g_bSwapHealthKitsWithBatteries)
		CollectAndRemove(CLS_HEALTHKIT, g_KitOrigin,  g_KitAngles,  g_KitCount);

	if(g_bSwapBatteriesWithHealthKits)
		RecreateEntities(CLS_HEALTHKIT, g_BatOrigin,  g_BatAngles,  g_BatCount);

	if(g_bSwapHealthKitsWithBatteries)
		RecreateEntities(CLS_BATTERY,   g_KitOrigin,  g_KitAngles,  g_KitCount);
}
...
public Task_ShowGameRules(const aPack[2], iUserId)
{
	...
	if(!(iBits & (1 << 1)) && g_fGaussDmgExponent > 0.0)
	{
		new szPart[128];
		formatex(szPart, charsmax(szPart), "- %L^n", iClient, "GAUSS_NERF_EXPONENT", g_fGaussDmgExponent);
		if(strlen(szPart) + strlen(szGameRules) < MAX_HUD_MESSAGE_LENGTH)
		{
			iBits |= (1 << 1);
			add(szGameRules, charsmax(szGameRules), szPart);
	...
	if(!(iBits & (1 << 2)) && g_fEgonMinDistanceDmgMultiplier > 0.0)
	{
		new szPart[128];
		new iPercentage = floatround(g_fEgonMinDistanceDmgMultiplier * 100);
		formatex(szPart, charsmax(szPart), "- %L^n", iClient, "EGON_NERF_PERCENTAGE_MIN", iPercentage);
		if(strlen(szPart) + strlen(szGameRules) < MAX_HUD_MESSAGE_LENGTH)
		{
			iBits |= (1 << 2);
			add(szGameRules, charsmax(szGameRules), szPart);
	...
	if(!(iBits & (1 << 3)) && g_fEgonMaxDistanceDmgMultiplier > 0.0)
	{
		new szPart[128];
		new iPercentage = floatround(g_fEgonMaxDistanceDmgMultiplier * 100);
		formatex(szPart, charsmax(szPart), "- %L^n", iClient, "EGON_NERF_PERCENTAGE_MAX", iPercentage);
		if(strlen(szPart) + strlen(szGameRules) < MAX_HUD_MESSAGE_LENGTH)
		{
			iBits |= (1 << 3);
			add(szGameRules, charsmax(szGameRules), szPart);
...
public HamForward_PlayerSpawn_Post(iClient)
{
	if(is_user_alive(iClient))
	{
		...
		if(g_iStartArmor > 0)
			set_user_armor(iClient, g_iStartArmor);
	}
}
...
public HamForward_PlayerTakeDamage_Pre(iVictim, iInflictor, iAttacker, Float:fDamage, iDamageBits)
{
	...
	switch(iUserWeapon)
	{
		case HLW_GAUSS:
		{
			...
			new Float:fAttackerChargeTime = get_ent_data_float(iAttacker, "CBasePlayer", "m_flStartCharge");
			new Float:fAttackTime = get_gametime();
			new Float:fFullChargeTime = GetFullChargeTime();
			
			new Float:fChargeDuration = fAttackTime - fAttackerChargeTime;
			new Float:fChargeRatio = fChargeDuration / fFullChargeTime;
			
			if (fChargeRatio > 1.0)
				fChargeRatio = 1.0;
			else if (fChargeRatio < 0.0)
				fChargeRatio = 0.0;
			
			fDamage = 10.0 + (190.0 * floatpower(fChargeRatio, g_fGaussDmgExponent));
			...
			{
				if(is_user_connected(iAiming)) // aiming at a player, no wall
					return HAM_HANDLED;
				else
				{ 
...
CollectAndRemove(const szClassName[], Float:vecOrigin[][3], Float:vecAngles[][3], &iItemCount)
{
	iItemCount = 0;
	new iEnt = -1;

	while ((iEnt = find_ent_by_class(iEnt, szClassName)) && iItemCount < MAX_ITEMS)
	{
		pev(iEnt, pev_origin, vecOrigin[iItemCount]);
		pev(iEnt, pev_angles, vecAngles[iItemCount]);
		iItemCount++;
		set_pev(iEnt, pev_flags, pev(iEnt, pev_flags) | FL_KILLME);
	}
}

RecreateEntities(const szClassName[], Float:vecOrigin[][3], Float:vecAngles[][3], iItemCount)
{
    new iClass = engfunc(EngFunc_AllocString, szClassName);

    for (new i = 0; i < iItemCount; i++)
    {
        new iEnt = engfunc(EngFunc_CreateNamedEntity, iClass);
        if (!pev_valid(iEnt))
            continue;

        engfunc(EngFunc_SetOrigin, iEnt, vecOrigin[i]);
        set_pev(iEnt, pev_angles, vecAngles[i]);
        dllfunc(DLLFunc_Spawn, iEnt);
        engfunc(EngFunc_DropToFloor, iEnt);
    }
}

stock Float:GetFullChargeTime()
{
	return 1.50;
}